...
public class Inheritance {
	public static void main(String[] args) {
		...
		<<오버라이딩>>
		상속받은 메소드의 내용을 재정의 하는것이다.
		상속받은 메소드와 리턴타입, 메소드명, 파라미터가 같아야한다.
		사옥받은 메소드보다 좁은 범위의 접근제어자로 변경할 수 없다.
		...
		자식 타입 > 부모타입 : Up-casting(생략가능)
		부모타임 > 자식타입 : Down-casting(생략불가)
		부모와 자식이 동일한 메소드가 있는 경우 자식의 메소드를 사용한다.
		부모와 자식이 동일한 멤버변수가 있는 경우 부모타입에서는 부모의 변수를 자식타입에서는 자식의 변수를 사용한다.
		
		<<접근제어자>>
		외부로부터 제이터를 보호받거나 감추기위해 사용한다.
		public : 접근에 제한이 없다.
		protected : 같은 패키지, 그리고 상속받은 클래스에서 접근이 가능한다.
